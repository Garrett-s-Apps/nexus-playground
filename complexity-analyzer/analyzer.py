...
class Reporter:
    ...
    def print_summary(self, all_metrics: List[FileMetrics]):
        ...
        for i, metrics in enumerate(all_metrics[:5], 1):
            status = self.color('🔴', self.RED) if metrics.max_complexity > self.thresholds['complexity'] * 1.5 else self.color('🟡', self.YELLOW) if metrics.max_complexity > self.thresholds['complexity'] else self.color('🟢', self.GREEN)
            filepath = Path(metrics.filepath).name
            print(f"  {i}. {status} {filepath:<40} complexity: {metrics.max_complexity}")
...
Examples:
  ...
  python3 analyzer.py --no-color
    ...
    if args.file:
        ...
        results = analyzer.analyze_directory(dirpath)
        ...
        if args.json:
            ...
            # Show details for top 5 most complex files
            if results:
